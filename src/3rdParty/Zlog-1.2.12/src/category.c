...
void zlog_category_del ( zlog_category_t * a_category )
{
    ...
    free ( a_category );
#ifdef _DEBUG_ZLOG
    zc_DEBUG_ZLOG ( "zlog_category_del[%p]", a_category );
#endif // _DEBUG_ZLOG
    return;
}
...
static int zlog_category_obtain_rules ( zlog_category_t * a_category, zc_arraylist_t * rules )
{
    ...
    if ( count == 0 )
    {
        ...
        {
#ifdef _DEBUG_ZLOG
            zc_DEBUG_ZLOG ( "category[%s], no match rules, use wastebin_rule", a_category->name );
#endif // _DEBUG_ZLOG
        ...
        {
            zc_warn ( "category[%s], no match rules & no wastebin_rule", a_category->name );
        }
    }
...
#ifdef _DEBUG_ZLOG
    zlog_category_profile ( a_category, ZC_DEBUG_ZLOG );
#endif // _DEBUG_ZLOG
