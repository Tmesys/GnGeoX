...
#else
#ifdef __MINGW32__
        if ( FlushFileBuffers ( _get_osfhandle ( a_rule->static_fd ) ) )
        {
            zc_error ( "fsync[%d] fail, errno[%d]", a_rule->static_fd, errno );
        }
#endif // __MINGW32__
...
static int zlog_rule_output_static_file_rotate ( zlog_rule_t * a_rule, zlog_thread_t * a_thread )
{
    ...
    {
#ifdef _DEBUG_ZLOG
        zc_DEBUG_ZLOG ( "one msg's len[%ld] > archive_max_size[%ld], no rotate",
...
/*******************************************************************************/
static int syslog_facility_atoi ( const char * facility )
{
                              ...
                              int * time_cache_count )
{
    ...
    {
#ifdef _DEBUG_ZLOG
        zc_DEBUG_ZLOG ( "no format specified, use default" );
#endif // _DEBUG_ZLOG
        ...
        case '>' :
            ...
            if ( STRNCMP ( file_path + 1, ==, "syslog", 6 ) )
            {
                ...
                if ( STRNCMP ( file_path + 1, ==, "stdout", 6 ) )
                {
                    ...
                    if ( STRNCMP ( file_path + 1, ==, "stderr", 6 ) )
                    {
...
#ifdef _DEBUG_ZLOG
    zlog_rule_profile ( a_rule, ZC_DEBUG_ZLOG );
#endif // _DEBUG_ZLOG
    return a_rule;
err:
...
void zlog_rule_del ( zlog_rule_t * a_rule )
{
    ...
    free ( a_rule );
#ifdef _DEBUG_ZLOG
    zc_DEBUG_ZLOG ( "zlog_rule_del[%p]", a_rule );
#endif // _DEBUG_ZLOG
    return;
}
...
int zlog_rule_match_category ( zlog_rule_t * a_rule, char * category )
{
    ...
    if ( STRCMP ( a_rule->category, ==, "*" ) )
    {
        ...
        if ( STRCMP ( a_rule->category, ==, category ) )
        {
            /* accurate compare */
            return 1;
        }
