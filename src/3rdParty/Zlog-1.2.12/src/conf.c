...
#ifdef _DEBUG_ZLOG
    zlog_conf_profile ( a_conf, ZC_DEBUG_ZLOG );
#endif // _DEBUG_ZLOG
    return a_conf;
err:
...
#else
#ifdef __MINGW32__
    local_time = localtime ( & ( a_stat.st_mtime ) );
#endif // __MINGW32__
#endif // __linux__
    ...
    switch ( *section )
    {
        case 1:
            ...
            if ( STRCMP ( word_1, ==, "strict" ) && STRCMP ( word_2, ==, "init" ) )
            {
                ...
                if ( STRCMP ( word_1, ==, "buffer" ) && STRCMP ( word_2, ==, "min" ) )
                {
                    a_conf->buf_size_min = zc_parse_byte_size ( value );
                }
                else
                    if ( STRCMP ( word_1, ==, "buffer" ) && STRCMP ( word_2, ==, "max" ) )
                    {
                        a_conf->buf_size_max = zc_parse_byte_size ( value );
                    }
                    else
                        if ( STRCMP ( word_1, ==, "file" ) && STRCMP ( word_2, ==, "perms" ) )
                        {
                            sscanf ( value, "%o", & ( a_conf->file_perms ) );
                        }
                        else
                            if ( STRCMP ( word_1, ==, "rotate" ) &&
