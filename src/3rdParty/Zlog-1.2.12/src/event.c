...
void zlog_event_del ( zlog_event_t * a_event )
{
    ...
    free ( a_event );
#ifdef _DEBUG_ZLOG
    zc_DEBUG_ZLOG ( "zlog_event_del[%p]", a_event );
#endif // _DEBUG_ZLOG
    return;
}
...
#else
#ifdef __MINGW32__
        zc_warn ( "gethostname fail, errno[%d]", WSAGetLastError() );
        strcpy ( a_event->host_name, "PS-XEM" );
#endif // __MINGW32__
#endif //__linux__
    }
...
#ifdef _DEBUG_ZLOG
    zlog_event_profile ( a_event, ZC_DEBUG_ZLOG );
#endif // _DEBUG_ZLOG
