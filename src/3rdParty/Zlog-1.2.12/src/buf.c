...
void zlog_buf_del ( zlog_buf_t * a_buf )
{
    ...
    free ( a_buf );
#ifdef _DEBUG_ZLOG
    zc_DEBUG_ZLOG ( "zlog_buf_del[%p]", a_buf );
#endif // _DEBUG_ZLOG
    return;
}
...
#ifdef _DEBUG_ZLOG
    zlog_buf_profile ( a_buf, ZC_DEBUG_ZLOG );
#endif // _DEBUG_ZLOG
    return a_buf;
    ...
    if ( nwrite >= 0 && nwrite < size_left )
    {
        ...
        if ( nwrite < 0 )
        {
            ...
            if ( nwrite >= size_left )
            {
                int rc;
#ifdef _DEBUG_ZLOG
                zc_DEBUG_ZLOG ( "nwrite[%d]>=size_left[%ld],format[%s],resize", nwrite, size_left, format );
#endif // _DEBUG_ZLOG
                ...
                if ( rc > 0 )
                {
                    ...
                    {
#ifdef _DEBUG_ZLOG
                        zc_DEBUG_ZLOG ( "zlog_buf_resize succ, to[%ld]", a_buf->size_real );
#endif // _DEBUG_ZLOG
                        va_copy ( ap, args );
                        size_left = a_buf->end_plus_1 - a_buf->tail;
                        nwrite = vsnprintf ( a_buf->tail, size_left, format, ap );
...
int zlog_buf_printf_dec32 ( zlog_buf_t * a_buf, uint32_t ui32, int width )
{
    ...
    size_t num_len = 0, zero_len = 0, out_len = 0;
    ...
    if ( ( q = a_buf->tail + out_len ) > a_buf->end )
    {
        int rc;
#ifdef _DEBUG_ZLOG
        zc_DEBUG_ZLOG ( "size_left not enough, resize" );
#endif // _DEBUG_ZLOG
