...
{
    struct bsUuCtx *luuCtx = ( struct bsUuCtx * ) calloc ( sizeof ( struct bsUuCtx ), sizeof( char ) );
    struct bStream *sOut;
    ...
    for ( i = 0; ; i++ )
    {
        ...
        if ( i + 1 >= b->mlen )
        {
            ...
            if ( b->mlen <= INT_MAX / 2 )
            {
                ...
                if ( b->mlen <= INT_MAX - 1024 )
                {
                    m = b->mlen + 1024;
                }
                else
                    if ( b->mlen <= INT_MAX - 16 )
                    {
                        m = b->mlen + 16;
                    }
                    else
...
struct bwriteStream *bwsOpen ( bNwrite writeFn, void *parm )
{
    ...
    ws = ( struct bwriteStream * ) calloc ( sizeof ( struct bwriteStream ), sizeof( char ) );
    if ( ws )
    {
...
unsigned int bcrc32 ( bstring b0 )
{
    const unsigned char *p;
    unsigned int crc = 0;
    unsigned int size = b0->slen;
...
unsigned int bhash ( bstring b0 )
{
    unsigned int hash = 0;

    for ( int i = 0; i < b0->slen; i++ )
    {
        hash = 65599 * hash + b0->data[i];
    }
