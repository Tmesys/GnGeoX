...
ErrMode:
                    ...
                    iter->error = 1;
                    v = errCh;
                    for ( i = iter->next + 1; i < iter->slen; i++ ) if ( ( iter->data[i] & 0xC0 ) != 0x80 )
                        {
                            break;
                        }
                    ofs = i - iter->next;
