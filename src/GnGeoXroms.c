...
static void open_nvram ( void )
{
    ...
    file_nvram_io = fopen ( fpath->data, "rb" );
    if ( file_nvram_io == NULL )
    {
        zlog_error ( gngeox_config.loggingCat, "Can not open file %s", fpath->data );
        return;
    }

    bdestroy ( fpath );

    fread ( neogeo_memory.sram, 1, 0x10000, file_nvram_io );
    fclose ( file_nvram_io );
}
/* ******************************************************************************************************************/
/*!
* \brief  Opens memcard.
...
static void open_memcard ( void )
{
    ...
    file_memcard_io = fopen ( fpath->data, "rb" );
    if ( file_memcard_io == NULL )
    {
        zlog_error ( gngeox_config.loggingCat, "Can not open file %s", fpath->data );
        return;
    }
    ...
    fread ( neogeo_memory.memcard, 1, 0x800, file_memcard_io );

    fclose ( file_memcard_io );
}
/* ******************************************************************************************************************/
/*!
* \brief  Saves nvram.
...
static void save_nvram ( void )
{
    ...
    for ( Sint32 i = 0xffff; i >= 0; i-- )
    {
        if ( neogeo_memory.sram[i] != 0 )
        {
            break;
        }
    }
    ...
    file_nvram_io = fopen ( fpath->data, "wb" );
    if ( file_nvram_io == NULL )
    {
        zlog_error ( gngeox_config.loggingCat, "Can not open file %s", fpath->data );
        return;
    ...
    fwrite ( neogeo_memory.sram, 1, 0x10000, file_nvram_io );
...
static void save_memcard ( void )
{
    ...
    file_memcard_io = fopen ( fpath->data, "wb" );
    if ( file_memcard_io == NULL )
    {
        zlog_error ( gngeox_config.loggingCat, "Can not open file %s", fpath->data );
        return;
    ...
    fwrite ( neogeo_memory.memcard, 1, 0x800, file_memcard_io );
